...
export default function Signup() {
  ...
  const validationSchema = yup.object({
    ...
    phone:yup.string().required("Phone is required").matches(phoneRegex, " We Accept Egyptian numbers only "),
...
const formik = useFormik({
 ...
 const PasswordStrength = CheckPasswordStrength(formik.values.password)
  ...
  return (
     <> 
     <PageMetaData title="Sign Up - Fresh Cart" description="Create a new Fresh Cart account to start shopping for fresh groceries." />
       <main className='py-12'>
         <div className="container grid lg:grid-cols-2 lg:gap-12">
           {/* left side */}
        <div className=' hidden lg:block  space-y-8 py-10'>
          <div className="welcome-msg">
            <h2 className='text-4xl font-bold'>Welcome to <span className='text-primary-500 '>FreshCart</span></h2>
             <p className='text-lg  mt-2'>
               Join thousands of happy customers who enjoy fresh groceries delivered  <br></br>right to their doorstep.
             </p>
          </div>
            ...
            <li>
             ...
             <li>
              ...
              <div>
                ...
                <p className='text-gray-500'>Your data and payment information are completely secure with us.</p>
              </div>
            </li>
          </ul>
          <div className="review  p-6 rounded-lg bg-white shadow-md space-y-2 ">
            <div className='flex items-center gap-3'>
              <img src= {profileimage} className='size-12 rounded-full' alt="Profile" />
                <div>
                  <h4>Sarah Johnson</h4>
               ...
               <blockquote className='text-gray-700 italic '>
                  <p>
                    "FreshCart has transformed my grocery shopping experience. The quality of the products is exceptional, and the convenience of having everything delivered to my doorstep saves me so much time. Highly recommend!"
                  </p>
                </blockquote>
          </div>
        </div>
        ...
        <div className='bg-white shadow-xl rounded-xl p-10 space-y-8'>
          ...
          <div className=' flex gap-2  *:flex *:items-center *:justify-center *:w-full *:gap-2 *:hover:bg-gray-100 *:transition:colors *:duration-300 '>
           ...
           <div className=' relative w-full h-0.5 bg-gray-300/30'>
            <span className='absolute left-1/2 top-1/2 -translate-1/2 px-4 bg-white'>or</span>
           </div>
          ...
          <div className="name flex flex-col gap-1">
            ...
            { formik.values.password &&  (
              ...
              <div className='bar rounded-xl overflow-hidden w-full h-1 bg-gray-200'>
                 <div className={`progress ${PasswordStrength.background} ${PasswordStrength.width}  h-full `}></div>
              </div>
            ...
            id="repPassword"
             ...
             />
             {
              formik.touched.rePassword && formik.errors.rePassword ? (
                <div className='text-sm text-red-600'>*{formik.errors.rePassword}</div>
              ) : ""
             }
          </div>
            ...
            <div className='flex items-center gap-2'>
              ...
              <label htmlFor="terms"> 
                I agree to <Link to={`/terms`} className='text-primary-600 underline'>Terms of Services</Link> and {" "} 
                <Link to={`/privacy-policy`} className='text-primary-600 underline'>Privacy Policy</Link>
                </label>
            </div>
                 {
                formik.touched.terms && formik.errors.terms ? (
                  <div className='text-sm text-red-600 mt-2  '>*{formik.errors.terms}</div>
                ) : ""
               }
             
            </div>

            <button type='submit' className='btn  w-full bg-primary-600 text-white flex justify-center items-center gap-2 hover:bg-primary-700 '>
               <FontAwesomeIcon icon={faUserPlus} />
               <span>Create My Account</span>
            </button>
        </form>
        <p className='text-center pt-8  border-t-2 border-gray-300/50'>
          Already have an account ? <Link to={`/login`} className='text-primary-600 underline'>Sign in</Link>
          </p>
        </div>
         </div>
       </main>
